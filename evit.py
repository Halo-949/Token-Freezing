...
class Attention(nn.Module):
    ...
    def forward(self, x, keep_rate=None, tokens=None, cls_attn_reco_in_layer=None, defr_flag=True, unfreeze_index=None):
        ...
        if self.keep_rate < 1 and keep_rate < 1 or tokens is not None:  # double check the keep rate
            ...
            if defr_flag is False and unfreeze_index is not None:
                B, N = unfreeze_index.shape
                unfreeze_index = unfreeze_index[:,1:N] - 1
                idx_new = idx[torch.isin(idx,unfreeze_index)]
                l = len(idx_new)
                idx_new = idx_new.reshape(1,l)
                index = idx_new.unsqueeze(-1).expand(-1, -1, C)  # [B=1, left_tokens, C]
                return x, index, idx_new, cls_attn, left_tokens
...
class Block(nn.Module):
    ...
    def forward(self, x, keep_rate=None, cls_attn_reco_in_layer=None, tokens=None, get_idx=False, defr_flag = True, unfreeze_index=None):
        ...
        if get_idx and index is not None:
            return x, n_tokens, cls_attn, idx
        return x, n_tokens, cls_attn, None
...
class EViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        super().__init__()
        self.img_size = img_size
        #print("EViT",keep_rate)
        if len(keep_rate) == 1:
            keep_rate = keep_rate * depth
        #print("EViT-after", keep_rate)
        self.keep_rate = keep_rate
        self.depth = depth
    ...
    def forward_features(self, x, keep_rate=None, cls_attn_reco=None, tokens=None, get_idx=False):
        _, _, h, w = x.shape
        ...
        if freeze is True:
            ...
            for i, blk in enumerate(self.blocks):
                if i in [9] and defreeze_flag == 1:  # defreeze
                    ...
                    defreeze_flag = 0
                    # b, n = unfreeze_index.shape
                    # b1, n1 = freeze_index.shape
                    # print("defreeze: i=", i, " n_unfreeze_index=", n, " b_unfreeze_index=", b, "\n")
                    # print("defreeze: i=", i, " n_freeze_index=", n1, " b_freeze_index=", b1, "\n")
                    if i == 6 and save_data is True:
                        np.savetxt('./x_unfreeze_tokens.csv', x_unfreeze_tokens.cpu().detach().numpy().reshape(-1, c_before), fmt='%.2f', delimiter=',')
                        np.savetxt('./x_freeze_tokens.csv', x_freeze_tokens.cpu().detach().numpy().reshape(-1, c_before), fmt='%.2f', delimiter=',')
                        np.savetxt('./unfreeze_index.csv', unfreeze_index.cpu().detach().numpy(), fmt='%.2f',
                                   delimiter=',')
                        np.savetxt('./freeze_index.csv', freeze_index.cpu().detach().numpy(), fmt='%.2f',
                                   delimiter=',')
                        np.savetxt('./x.csv', x.cpu().detach().numpy().reshape(-1, c_before), fmt='%.2f',
                                   delimiter=',')
                        print("data saved!\n")
                        sys.exit()
    ...
    def forward(self, x, keep_rate=None, cls_attn_reco=None, tokens=None, get_idx=False):
        x, _, idxs, cls_att_reco = self.forward_features(x, keep_rate, cls_attn_reco, tokens, get_idx)
        if self.head_dist is not None:
            ...
            x = self.head(x)
        if get_idx:
            return x, idxs, cls_att_reco
        return x, cls_att_reco
        # if get_idx:
        #     return x, idxs
        # return x
